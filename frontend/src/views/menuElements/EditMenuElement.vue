...
export default {
  ...
  methods: {
    ...
    save() {
        ...
        axios.post(  '/api/menu/element/update?token=' + localStorage.getItem("api_token"),
          data
        )
        .then(function (response) {
            self.name = '';
    ...
    getData() {
      let self = this;
      axios.get(  '/api/menu/element/edit?token=' + localStorage.getItem("api_token") + '&id=' + self.$route.params.id )
      .then(function (response) {
