  ...
  <div class="wrapper">
    ...
    >
      <template slot="header">
        <h5 class="modal-title"> Give  RouteNumber to a driver </h5>
  ...
  export default {
    ...
    methods:{
      ...
      giveOrder (id) {
        ...
        let self= this;
        this.items = [];
        axios.post(  '/api/assignRouteToDriver?token='+localStorage.getItem("api_token"),{
          route_number_id: self.routeNumberId,
          fleet_id: id,
        })
          .then(function (response) {
            self.loading = false;
