<template>
  <CRow>
      ...
      <transition name="slide">
        <CCard>
          ...
          <CCardBody>
            <h4>
              {{ $t("total_balance") }}
              <CBadge :color="getWalletBadge(balance)">{{ balance }}</CBadge>
            </h4>
            <CButton
              v-if="role != 'user'"
              class="m-3"
            ...
            >Withdraw Money From this wallet</CButton
            >
            <CButton class="m-3" color="success" @click="depositModal = true"
            >Deposit Money in this wallet</CButton
            >
            ...
            <CCol sm="12">
              ...
              >
                ...
                <template #amount="{ item }">
                  <td>
                    <CBadge :color="getWalletBadge(item.amount)">{{
                        item.amount
                      }}</CBadge>
                  </td>
                </template>
    ...
    <CModal :title="`Withdraw Fund From ${balance}`" :show.sync="withdrawModal">
      <CInput
        type="text"
        label="Amount"
        placeholder="Amount"
    ...
    <CModal :title="`Deposit `" :show.sync="depositModal">
      <CInput
        type="text"
        label="Amount"
        placeholder="Amount"
      ...
      axios
        ...
        .then(function (response) {
          ...
          self.items = self.items.concat(
            response.data.wallet.transactions.data
          );
