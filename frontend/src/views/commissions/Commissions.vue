  ...
  <CRow>
    <CCol col="12" xl="12">
      ...
      <transition name="slide">
        <CCard>
          <CCardHeader>
            <span :style="(color = 'red')">Delivery Cost</span>
          </CCardHeader>
          ...
          <CCardBody>
            ...
            <CCol sm="12">
              <CDataTable
                ...
                :loading="loading"
              >
                <template #default_distance="{item}">
                  <td>
                    {{ item.distance }} KM
                  </td>
                </template>
                <template #default_price="{item}">
                  <td>
                    {{ item.price }} SR
                  </td>
                </template>

                <template #price_per_more_1KM="{item}">
                  <td>
                    {{ item.price_more_kilo }} SR
                  </td>
                </template>
                <template #max_delivery_cost_per_order="{item}">
                  <td>
                    {{ item.max_cost }} SR
                  </td>
                </template>
...
<!--                    <CBadge :color="getBadge(item.type2)"  style="padding: 3px ; border-radius: 10px ;text-align: center">{{-->
<!--                      item.type2-->
<!--                    }}</CBadge>-->
...
export default {
  ...
  methods: {
    ...
    editLink(id) {
      return `delivery-cost/${id.toString()}/edit`;
    },
    fleetLink(id) {
      return `delivery-cost/${id.toString()}/fleet`;
    },
    showLink(id) {
      return `delivery-cost/${id.toString()}/show`;
    },
    createFleet() {
      this.$router.push({ path: "delivery-cost/create" });
    },
    editCommission(id) {
